...
<body>
    <%
        ...
        if(user==null){
          ...
          TransactionService transactionSvc = new TransactionService(pool.getConnection());
          users = userSvc.requestAll("surname ASC, name ASC");
          transactions = transactionSvc.requestAll("fecha DESC, id ASC");
        }
...
if (transactions != null) {
    for (int i = 0; i < transactions.size(); i++) {
        ...
        out.print(String.format(
                ...
                "    <td>%.2f€</td>" +
                "    <td>%s</td>" +
                "    <td><a href=\"edit-transaction.jsp?id=" + transaction.getIdTransaccion() +
                      "&idCuenta=" + transaction.getIdCuentaOrigen() +
                      "&tipo=" + transaction.getTipo() +
                      "&cantidad=" + transaction.getCantidad() +
                      "\"><i class=\"bi bi-pen\" style=\"color:green!important\"></i></a></td>" +

                "    <td><a href=\"#\" onclick=\"confirmDelete('eliminarTransaccion.jsp?idTransaccion=" + transaction.getIdTransaccion() + "')\"><i class=\"bi bi-trash\" style=\"color:red!important\"></i></a></td>" +                 
                "</tr>",
                transaction.getIdTransaccion(),
                transaction.getIdCuentaOrigen(),
                transaction.getTipo(),
                transaction.getCantidad(),
                transaction.getFecha().toString()));
    }
}
%>
